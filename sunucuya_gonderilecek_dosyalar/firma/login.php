    ...
    if (empty($identifier) || empty($password)) {
        ...
        if (filter_var($identifier, FILTER_VALIDATE_EMAIL)) {
            ...
            $sql = "SELECT * FROM kullanicilar 
                    ...
                    AND aktif = 1 
                    AND rol IN ('super_admin', 'firma_sahibi', 'broker')";
        } else {
            // Telefon ile giriş (sadece rakamları al)
            $phone = preg_replace('/[^0-9]/', '', $identifier);
            $sql = "SELECT * FROM kullanicilar 
                    WHERE telefon = :identifier 
                    AND aktif = 1 
                    AND rol IN ('super_admin', 'firma_sahibi', 'broker')";
            $identifier = $phone;
        }
...
<head>
    ...
    <title>Firma Girişi - emlakimza.com</title>
...
<body>
    <div class="login-container">
        ...
        <div class="text-center">
            <span class="firma-badge">
                <i class="bi bi-star-fill"></i> FİRMA SAHİBİ
            </span>
        </div>

        <h4>emlakimza.com</h4>
        <p class="subtitle">Firma yönetim paneli girişi</p>
        ...
        <div class="text-center mt-3">
            <small class="text-muted">
                Danışman mısınız?
                <a href="../danisman/login.php" class="text-decoration-none">Danışman girişi</a>
            </small>
        </div>
